...
warren_select <- warren %>% 
  ...
  #removing Devens - its an army base not a municipality
...
muni_transactions <- warren_select %>% 
  group_by(municipal) %>% 
...
warren_municipal <- full_join(warren_all, muni_transactions, by = 'municipal') %>% 
