...
- we are matching 60,595 non-llc/llp entities to networks
...
total_llcs21 <- hfp_networks |> 
  filter((buyer_llc_ind == 1 | buyer_llp_ind == 1) & year == 2021) |> 
  select(buyer1_adj) |> 
  distinct()
# 6,003

matched_llcs21 <- hfp_networks |> 
  filter((buyer_llc_ind == 1 | buyer_llp_ind == 1) & year == 2021 & network_flag == 1) |> 
  select(buyer1_adj) |> 
  distinct()
# 3,504
```
...
o_non_matches21h <- hfp_joined |> 
  filter(n_network_flag == 0 & year == 2021) |> 
  count(investor_type_purchase) |> 
  rename(no_network_count = n) 

o_network_matches21h <- hfp_joined |> 
  filter(n_network_flag == 1 & year == 2021) |> 
  count(investor_type_purchase) |> 
  rename(network_count = n)
